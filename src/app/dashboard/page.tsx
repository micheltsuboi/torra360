...
export default async function DashboardIndex() {
  ...
  return (
    <div className="flex flex-col pt-12 pb-24 max-w-[1500px] mx-auto min-h-full">
      ...
      <div className="mb-20">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="glass-panel p-0 flex flex-col relative overflow-hidden border-t-2 border-[--primary]/30 shadow-2xl">
            ...
            <div className="p-10 flex-1 flex flex-col min-h-[300px] relative">
              ...
              <div className="mb-12">
                 ...
                 <div className="flex items-baseline gap-2 mt-2">
                   <span className="text-6xl font-serif text-[--primary] tracking-tighter">{averageYieldStr.split('.')[0]}</span>
                   <span className="text-2xl font-serif text-[--primary] opacity-40">.{averageYieldStr.split('.')[1]}%</span>
                 </div>
              </div>

              <div className="flex items-end justify-between gap-6 h-[100px] mt-auto relative z-10">
                {latestFive.length > 0 ? (
                  latestFive.map((roast: any, idx: number) => {
                    ...
                    return (
                      <div key={roast.id || idx} className="flex-1 flex flex-col items-center group">
                        <div className="relative w-full flex flex-col justify-end h-[85px] bg-white/[0.03] rounded-t-xl border-x border-t border-white/5 overflow-hidden group-hover:bg-white/[0.08] transition-all duration-300">
                          <div className={`absolute top-2 w-full text-center font-bold text-[10px] ${textClass} z-20`}>
                            {yieldP.toFixed(0)}%
                          </div>
                          <div 
                            className={`w-full ${colorClass} transition-all duration-1000 ease-out relative shadow-[0_0_20px_rgba(0,0,0,0.5)]`} 
                            style={{ height: `${height}%` }}
                          >
          ...
          <div className="glass-panel p-0 flex flex-col overflow-hidden border-t-2 border-[--primary]/30 shadow-2xl">
            ...
            <div className="p-0 flex flex-col overflow-y-auto max-h-[380px] scrollbar-thin">
              {latestFive.length > 0 ? (
                latestFive.map((roast: any, idx: number) => (
                  <div key={roast.id || idx} className="flex items-center gap-6 px-10 py-6 border-b border-white/5 hover:bg-white/[0.05] transition-colors group">
                    <div className="w-14 h-14 rounded-2xl border border-white/10 bg-black/40 flex items-center justify-center shrink-0 group-hover:border-[--primary]/50 transition-all duration-300 shadow-xl">
                      <span className="text-[12px] font-bold text-[--primary]">TR</span>
                    </div>
                    <div className="flex-1 min-w-0">
                       <h4 className="text-[14px] font-bold text-[--foreground] truncate uppercase tracking-wider">{roast.green_coffee?.name || 'Lote de Café'}</h4>
                       <p className="text-[11px] font-normal text-[--secondary-text] opacity-40 capitalize tracking-tight mt-1">Sessão em {formatDate(roast.date)}</p>
                    </div>
                    <div className="text-right shrink-0">
                      <div className="text-2xl font-bold text-[--success] leading-none tracking-tight">{roast.qty_after_kg.toFixed(1)}kg</div>
                      <div className="text-[10px] font-normal text-[--secondary-text] opacity-30 uppercase tracking-widest mt-2">Peso Final</div>
                    </div>
                  </div>
                ))
              ) : (
      ...
      {/* BLOCO CENTRAL: Cartões de Resumo (4 colunas) - ESPAÇO EXPLÍCITO ABAIXO */}
      <div className="mb-20">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {[
            ...
            <div key={idx} className={`glass-panel p-0 border-t-2 border-t-${item.color}/40 relative overflow-hidden group shadow-xl`}>
               <header className="px-6 py-5 border-b border-white/5 wood-texture bg-black/60 flex justify-between items-center">
                 <div className="flex items-center gap-3">
               ...
               <div className="px-8 py-8">
                 <div className="flex items-baseline gap-2">
                   <span className={`text-2xl font-bold text-[--foreground] tracking-tight ${idx > 1 ? 'text-4xl' : ''}`}>{item.value}</span>
                   {item.labelSuffix && <span className="text-[12px] font-medium text-[--secondary-text] opacity-40 uppercase tracking-widest">{item.labelSuffix}</span>}
                 </div>
                 <div className="mt-8 pt-6 border-t border-white/5 flex flex-col gap-2">
                   <span className="text-[10px] font-normal text-[--secondary-text] opacity-50 uppercase tracking-[0.2em]">{item.sub}</span>
                   <span className={`text-[12px] font-bold uppercase tracking-wider text-${item.trendColor} ${idx === 1 ? 'opacity-60' : ''}`}>{item.trend}</span>
                 </div>
               </div>
            </div>
          ))}
      ...
      <div className="mb-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
          ...
          <div className="glass-panel p-0 relative overflow-hidden border-t-2 border-[--success]/30 shadow-2xl">
            ...
            <div className="p-12 relative z-10 flex flex-col min-h-[220px]">
                   ...
                   {recentRoasts.length > 1 && (
                     <>
                       ...
                       {recentRoasts.slice(0, 10).reverse().map((r, i) => {
                          ...
                          const x = (i / (Math.min(10, recentRoasts.length) - 1)) * 100;
                          return (
                            <circle key={i} cx={x} cy={y} r="2.5" fill="var(--background)" stroke="var(--success)" strokeWidth="2" className="transition-all duration-300" />
                          )
                       })}
                     </>
                   )}
               ...
               <div className="mt-14 flex justify-center gap-20">
                  <div className="flex flex-col items-center">
                    ...
                    <span className="text-2xl font-bold text-[--danger] mt-2">74.2%</span>
                  </div>
                  <div className="flex flex-col items-center">
                    <span className="text-[11px] uppercase font-medium text-[--secondary-text] opacity-50 tracking-widest">Máxima</span>
                    <span className="text-2xl font-bold text-[--success] mt-2">92.8%</span>
                  </div>
               </div>
            </div>
          </div>
          ...
          <div className="glass-panel p-0 flex flex-col border-t-2 border-[--primary]/30 shadow-2xl">
            <header className="px-8 py-5 border-b border-white/10 wood-texture bg-black/60 flex justify-between items-center relative z-10">
             ...
             <div className="p-0">
               <div className="flex justify-between items-center px-12 py-8 border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                 <div>
                   <p className="text-[16px] font-bold text-[--foreground] uppercase tracking-wider">Café Verde</p>
                   <p className="text-[11px] font-normal text-[--secondary-text] opacity-50 uppercase tracking-widest mt-2">Matéria-prima em sacas</p>
                 </div>
                 <div className="text-right">
                   <span className="text-4xl font-bold text-[--foreground] tracking-tighter">{totalGreenStock.toFixed(1)}</span>
                   <span className="text-[14px] font-medium opacity-20 tracking-widest uppercase ml-4">kg</span>
                 </div>
               </div>
               
               <div className="flex justify-between items-center px-12 py-8 border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                 <div>
                   <p className="text-[16px] font-bold text-[--primary] uppercase tracking-wider">Café Embalado</p>
                   <p className="text-[11px] font-normal text-[--secondary-text] opacity-50 uppercase tracking-widest mt-2">Produtos finalizados</p>
                 </div>
                 <div className="text-right">
                   <span className="text-4xl font-bold text-[--primary] tracking-tighter">{totalRoastedUnits}</span>
                   <span className="text-[14px] font-medium opacity-20 tracking-widest uppercase ml-4">unid</span>
                 </div>
               </div>

               <div className="mx-12 my-10 p-6 bg-black/60 rounded-2xl border border-white/10 flex items-center justify-between shadow-inner">
                  <div className="flex items-center gap-6">
                    <div className="p-4 bg-[--success]/10 rounded-full border border-[--success]/20 shadow-[0_0_20px_rgba(34,197,94,0.15)]">
                    ...
                    <div>
                      ...
                      <span className="text-[14px] font-bold text-[--success] uppercase tracking-[0.1em] mt-1 block">Operação Estabilizada</span>
                    </div>
                  </div>
               </div>
             </div>
